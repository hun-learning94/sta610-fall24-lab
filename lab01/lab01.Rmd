...
\begin{quote}
    \textbf{Q: Write down the form of a design matrix $\boldsymbol{X}$.} 
\end{quote}
...
$$
\begin{quote}
    \textbf{Q: Re-express the above $H_0$ using $\beta$.} 
\end{quote}
...
\begin{enumerate}
  \item One can see that $SST = \|(I - P_{1})y\|^2 = y^T(I - P_{1})y$, $SSA = y^T(P_{Z} - P_{1})y$ and $SSE = y^T(I - P_{Z})y$.
...
\begin{quote}
    \textbf{Q: Perfom f-test of treatment effect as learned in the class and check that the test statistics is the same.} 
\end{quote}
...
\begin{quote}
    \textbf{Q: Write down the covariance matrix $\Sigma_j$ as follows:}
$$
(\Sigma_j)_{kl}=
\begin{cases}
?\quad(k=l)\\
...
\begin{quote}
    \textbf{Q: What is $Cov(y_{i1},y_{i2})$? Using this, how can we write $\Sigma$?}
\end{quote}
...
\begin{quote}
    \textbf{Q: Write down the joint model for $\boldsymbol{y}$}
\end{quote}
